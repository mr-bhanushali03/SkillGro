...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var username = greetingElement.getAttribute("data-username");

    greetingElement.innerHTML =
        "Good " +
        greetingTime.charAt(0).toUpperCase() +
        greetingTime.slice(1) +
        ", " +
        username;
});

flatpickr("#countdown-picker", {
    enableTime: true,
...
function updateCountdown() {
    ...
    const flatpickr_hour = document.querySelector(".flatpickr-hour");
    const flatpickr_minute = document.querySelector(".flatpickr-minute");
    const flatpickr_am_pm = document.querySelector(".flatpickr-am-pm");
    const currentDate = new Date();

    // Update the input field with the current date and time
    countdownPicker.value = currentDate.toLocaleString('en-US', { day: 'numeric', month: 'short', year: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true });
    flatpickr_hour.value = currentDate.getHours() % 12 || 12;
    flatpickr_minute.value = currentDate.getMinutes();
    flatpickr_am_pm.value = currentDate.getHours() >= 12 ? "PM" : "AM";

    // Update every second
    setTimeout(updateCountdown, 1000);
}

// Initial call to start updating every second
updateCountdown();